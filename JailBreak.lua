...
-- تنظيف أي مخلفات من النسخ السابقة
if CoreGui:FindFirstChild("KyperMobileAim") then CoreGui.KyperMobileAim:Destroy() end
if CoreGui:FindFirstChild("KyperESP_Folder") then CoreGui.KyperESP_Folder:Destroy() end
if CoreGui:FindFirstChild("KyperESP_2D") then CoreGui.KyperESP_2D:Destroy() end

pcall(function()
    local oldSetCore
    oldSetCore = hookfunction(StarterGui.SetCore, function(self, name, data)
        if name == "SendNotification" and type(data) == "table" then
            if data.Title and type(data.Title) == "string" and data.Title ~= "KyperHub" and data.Title ~= "KyperHub Security" then
                return 
            end
        end
...
-- ==========================================
-- Custom UI Library (Mini Auto Farm Panel)
-- ==========================================
local Kyper = {}
...
function Kyper:CreateWindow(titleText)
    ...
    TabLine.Position = UDim2.new(0, 15, 0, 40)
    ...
    function WindowObj:CreateSection(secName)
...
-- ==========================================
-- Setup Kyper Auto Farm
-- ==========================================

local Window = Kyper:CreateWindow("Kyper - JailBreak Auto Farm")

local SecInfo = Window:CreateSection("Information")
SecInfo:CreateLabel("Welcome back, " .. player.Name .. "!")
...
SecRob:CreateButton({Name = "Start Auto Farm", Color = Color3.fromRGB(100, 255, 100)}, function()
    if isFarming then return end
    isFarming = true
    sendNotification("KyperHub", "Auto Farm Started")
                ...
                for _, v in pairs(workspace:GetChildren()) do
                    if v:IsA("BasePart") then
                        if (v.Position - hrp.Position).Magnitude < 12 then
                            ...
                            if logo then logo:Destroy(); v.Name = "KyperPlatform" end
                        end
    ...
    task.spawn(function()
        ...
        task.wait(2.5)
        if guiTargetCore:FindFirstChild("KyperUI") then guiTargetCore.KyperUI:Destroy() end
        pcall(function()
...
SecRob:CreateButton({Name = "Stop & Rejoin Server", Color = Color3.fromRGB(255, 80, 80)}, function()
    if not isFarming then return end
    isFarming = false
    sendNotification("KyperHub", "Stopping & Rejoining...")
